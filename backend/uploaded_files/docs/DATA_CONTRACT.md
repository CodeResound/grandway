...
- **Ownership is six nullable foreign keys, not a `content_type`/`object_id` pair.** The loose-pair form would let a file point at a record that never existed or has since gone, with nothing in the database to notice — the opposite of what an accountability system wants. `PROTECT` on all six matches every existing cross-app FK in the project. The sixth, `signatory`, was added on 2026-09-10 and is the worked proof that this shape extends the way it claimed: one nullable column, one entry in `OWNER_FIELDS`, and one migration that drops and re-adds the constraint.
...
| Ownership | Exactly one of six business records, enforced by a database constraint |
...
- `uploaded_file_single_owner` — a six-way disjunction requiring exactly one owner FK non-null and the other five null. Written as an explicit `Q` disjunction rather than an arithmetic null-count so it evaluates identically on PostgreSQL and on the SQLite used by the test suite; **do not "simplify" it to `num_nonnulls(...) = 1`**, which is PostgreSQL-only and would stop the test suite exercising the rule.

  **The model derives it from `OWNER_FIELDS`; the migration inlines it expanded.** The two can therefore silently disagree — an owner added to the tuple without a migration that drops and re-adds the CHECK leaves a constraint permitting two owners for the new column. `tests/test_services.py::test_the_constraint_covers_every_pair_of_owners` sweeps all 15 pairs for exactly this reason. Note also that a future edit here must emit `RemoveConstraint`/`AddConstraint`, never Django 5.2's `AlterConstraint`, whose `database_forwards` is a no-op and would leave the old CHECK on the PostgreSQL table while migration state claimed otherwise.
...
**What this app references:** six business models by foreign key, `authenticate.User`, and `audit.services.record_event`, plus each owner app's own `get_<owner>_by_id` selector (§4 permits importing another app's `selectors.py`; the alternative — assigning a raw id to a foreign key — would push a nonexistent owner all the way to an `IntegrityError` and surface as a 500 naming no field). No rule, enum, or validator is shared with another app.
...
**Two apps reference this one, both the same way.** `checklists.ChecklistItem.evidence_file` cites the file proving a requirement was met, and `document_templates.Signatory.signature_file` names the signature image that renders on a certificate. Both are nullable `PROTECT` foreign keys resolved through a function-local selector import.

**`document_templates` is the project's only bidirectional app pair.** This app imports its `selectors` at module level for the owner lookup; it imports this app's `services` function-locally. The invariant keeping that acyclic is written at both import sites: **neither app's `selectors.py` may import its own `services.py`.** Break it and Django fails at startup with a "partially initialized module" traceback naming neither app's real problem.

**Everywhere else the direction is still one-way.** `applicants` has no photograph field, `offers` has no attachment field, `documents` has no attachment field, `document_history` has no generated-file field, and `clients.logo_url` is still a plain URL. A file knows its owner; an owner learns about its files only through this app's list endpoint filtered by that record's id. A reverse pointer is what an app adds when *which* file is the answer to a question rather than merely *some* file among several — which is what `document_templates` needed and `applicants` still lacks.
