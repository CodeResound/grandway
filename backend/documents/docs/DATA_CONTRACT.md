...
**Not enforced by this backend.** Reproduced from `frontend_data-contract.md` so a maintainer can see what the body is expected to hold. Every shape is open: unknown keys are preserved. The authoritative version is the frontend's own contract, and `document_templates` will eventually own it properly.
...
**Signature references.** `content.instructorId` and `content.directorId` on certificate templates are foreign keys into a Signature table owned by the unbuilt `document_templates` app. They are stored and returned as opaque strings. **Nothing validates them**, and a document may reference a signatory that never existed.
