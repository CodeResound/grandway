...
**`uploaded_files` is the first app whose edges are all inbound-facing and none reciprocated.** It depends on five business apps; **not one of them depends on it.** No module holds a foreign key to a file, calls into it, or returns a file reference of any kind, so every one of them functions exactly as it did before this app existed. The practical consequence for a client is in §10: a files panel is always a second call.
...
- **Files are stored now, but no module points at one.** `uploaded_files` shipped on 2026-07-24 and holds `PROTECT` foreign keys to applicants, journeys, offers, documents, and print snapshots — so a passport scan, an offer letter, a document attachment, and a generated PDF all have somewhere to live. **What no module has is a reference in the other direction.** An applicant payload carries no photograph, an offer carries no letter, a document carries no attachment, and a snapshot carries no file — every files panel in the product is a *second* call to `/api/v1/files/?<owner>=<id>` that the screen joins itself. Adding a reverse pointer to any of those apps is additive and is a separate session each.
