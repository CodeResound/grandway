...
- **Access models differ per app and cannot be assumed. There are now three distinct shapes:**
  ...
  | **Admin-only** | **`documents`**, **`document_history`** | **no** | **no** | **no** |
  ...
  **`documents` and `document_history` are the cases most likely to break a client**, because they are the only apps where a Lead Manager cannot even list. Both modules' panels must be *hidden* for them, not rendered read-only or shown empty — an empty panel asserts "this applicant has no documents", which is false. Each app's `INTEGRATION.md` §3 states its own rule; that is the consumer-facing source. `documents/docs/SECURITY.md` §1 carries the same rule plus the reasoning **for both apps**, and is maintainer-facing — useful, but not part of the contract set in §6. `document_history` publishes no `SECURITY.md` of its own precisely because it adds no security decision of its own.
- **Two named document domains are still unbuilt, and both `documents` and `document_history` are incomplete without them.** `document_templates` (template definitions and signatory records) and `uploaded_files` (supporting files, and any generated PDF) have no code and no concept file. A client can now create, edit, archive, restore, **print, review print history, reprint, and recover** — but still cannot resolve a signatory reference or attach a file, and **no generated PDF can be stored or referenced anywhere**: a snapshot has no file field at all. See `documents/docs/INTEGRATION.md` §2 and `document_history/docs/INTEGRATION.md` §9.
