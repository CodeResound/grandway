...
| 1.14.0 | 2026-09-10 | AI (Claude Opus 5) | No new app. `uploaded_files` gained a **sixth owner type**, `signatory`, so `document_templates` can store real signature images — which falsified five standing claims in §7/§10 and one in §6, all corrected here: that `document_templates` holds "no database relation to anything", that "exactly one module points back at" a file, that `document_templates.signature_image_url` "was deliberately not migrated … there is nowhere to attach [it] even by hand", that `checklists`' `evidence_file` was "the first hard constraint of its kind", and that `document_templates` publishes no `SECURITY.md` (it now does — the first artefact in the project whose threat model is forgery rather than disclosure). §7 also records the project's **first mutual edge**. The `signatory` owner type is Admin-only in the ledger, extending §10's per-record visibility rule from two owner types to three |
...
| `document_templates` | `/api/v1/document-templates/` | The signatory library certificate documents name, and the catalogue of template slugs the document picker offers. **Stores what is *offered*, not what is *rendered*** — the templates themselves are frontend code, and no endpoint here describes how a document draws itself. Signature images **are** stored, through a multipart route that delegates to `uploaded_files`. **Admin only, reads included**, like the other two document modules — though this is the only one holding no applicant data. Retired, never deleted. The catalogue is **advisory**: `documents` does not consult it | `document_templates/docs/INTEGRATION.md`, `document_templates/docs/SECURITY.md` |
| `uploaded_files` | `/api/v1/files/` | **The platform's file ledger — the only place in Grandway where bytes are stored.** Upload, category, verification state, replacement chains, and archival, for files belonging to an applicant, journey, offer, document, print snapshot, or signatory. **The bytes are private**: `MEDIA_URL` is unset, nothing is web-served, and a file leaves only through an authenticated, audited download endpoint. Nothing is ever deleted. Shared read/write with **Admin-only** verify, archive, and restore — and a file **inherits the visibility of its owner**, so `document`-, `snapshot`-, and `signatory`-owned files are Admin-only entirely | `uploaded_files/docs/INTEGRATION.md` |
...
- `document_templates` → `core` (framework), `documents` (**three Python imports, no database relation** — the `DocumentFamily` enum, the template-key slug validator, and the key/family agreement rule, the last being a **service call** made at request time, so the catalogue can never accept a pairing `documents` would reject), `uploaded_files` (**FK + service call** — `Signatory.signature_file` names the stored signature image, and the signature route calls that app's `upload_file`/`replace_file`; this is the module's only database relation outside itself), `authenticate` (framework; FK — `created_by` on both models), `audit` (service call — every mutation appends one event, **unredacted**, since this module holds no applicant data). **No edge to `applicants` or `document_history` in either direction.**
- `uploaded_files` → `core` (framework), **the local filesystem** (infrastructure — `MEDIA_ROOT` is the byte store, not web-served), `applicants` / `applicant_journeys` / `offers` / `documents` / `document_history` / `document_templates` (**FK + service call, all six** — a file belongs to exactly one of them, and each owner id is resolved through that app's own selector on upload; the `document_templates` edge is the only one that runs both ways), `authenticate` (framework; FK — `uploaded_by`, plus three `SET_NULL` lifecycle actors), `audit` (service call — every write **and every download** appends one event). **The most-connected app in the graph, and the only one depending on infrastructure rather than only on other apps.**
...
**One edge is mutual, and it is the only one.** `document_templates` ⇄ `uploaded_files`: the ledger resolves a signatory owner through that module's selector, and that module stores signature bytes through the ledger's service. Every other edge in this graph runs one way. The pair is acyclic at import time only because one direction is deferred to call time; both apps' source carries the invariant that keeps it so.
...
**The three document modules form a stack, and every edge between them is weaker than it looks.** `documents` is the centre and depends on neither of the others. `document_history` holds real foreign keys to it and performs one write through its service. `document_templates` holds **no database relation to anything in the document stack** — it imports three Python objects from `documents` (the family enum, the slug validator, the key/family rule) so the two cannot disagree about what a valid template key is, and that is the entire coupling within the stack. Its one database relation points *outside* the stack, at `uploaded_files`, for signature images. The practical consequence for a consumer:
...
- **Access models differ per app and cannot be assumed. There are now six distinct shapes:**
  ...
  **`uploaded_files` additionally scopes by the owning *record*, and it is the only app that does.** (Distinct from `notifications`, which scopes by the calling *user* — see the row below it. The two are different mechanisms: a file's visibility is inherited from what it is attached to, a notification's from who it was addressed to.) A file inherits the visibility of the record it belongs to, so a file owned by a `document`, a print snapshot, or a **signatory** is Admin-only in every respect — invisible in lists and reported as **404** on every per-file route for a Lead Manager. Without that rule the file ledger would have been a side door around the Admin-only row above; the leak was real, and was found by a §19.5 review rather than by design. The `signatory` entry was added by design rather than discovery, and note that it is Admin-only for a *different* reason from the other two: a signature image is not confidential — it appears on every certificate the consultancy has issued — but a print-ready copy is what lets someone forge one, and whoever can replace it controls what every certificate appears to be signed by. **If Lead Managers are ever given document access, that app's `ADMIN_ONLY_OWNER_TYPES` must change in the same session** or its attachment panels will stay dark while the documents themselves become visible — and `signatory` should be re-argued separately rather than swept along, because the forgery argument survives widening document access.
  ...
  **The three document modules are the cases most likely to break a client**, because they are the only apps where a Lead Manager cannot even list. All three modules' panels must be *hidden* for them, not rendered read-only or shown empty — an empty panel asserts "this applicant has no documents", which is false. Each app's `INTEGRATION.md` §3 states its own rule; that is the consumer-facing source. `documents/docs/SECURITY.md` §1 carries the reasoning for the first two, and is maintainer-facing — useful, but not part of the contract set in §6. `document_history` publishes no `SECURITY.md`, because it adds no security decision of its own. **`document_templates` gained one on 2026-09-10**, when it began accepting uploaded signature images — the first artefact in the project whose threat model is forgery rather than disclosure.

  **`document_templates` is Admin-only for a different reason than the other two, and that is worth knowing before anyone relaxes it.** It holds **no applicant data at all** — a signatory is a staff member's name and their signature image; a template is a slug and a label. Its risk ratings are `low`/`medium` throughout, against `high` for the endpoints that return a document body — with one exception, `signatory.upload_signature`, which is `high` because it is the only route in that module that writes bytes, and those bytes are what make an issued certificate look authoritative. The Admin-only rule is inherited from its *consumer*: a Lead Manager who cannot open a document workspace has no way to use a signatory picker. If document access is ever widened, that module should follow in the same change rather than being re-argued from scratch.
- **Files are stored, and exactly two modules point back at one.** `uploaded_files` holds `PROTECT` foreign keys to applicants, journeys, offers, documents, print snapshots, and signatories. In the other direction there are two references: a `checklists` item may cite a file as `evidence_file`, proving a requirement was met, and a `document_templates` signatory names its `signature_file`, the image that renders on a certificate. **Every other reverse pointer is still absent** — an applicant payload carries no photograph, an offer carries no letter, a document carries no attachment, and a snapshot carries no file, so those files *panels* remain a second call to `/api/v1/files/?<owner>=<id>` that the screen joins itself.

  **What earns a reverse pointer is worth stating, because two apps now have one and four do not.** It is not "this app has files" — it is *which* file is the answer to a question. A signatory renders exactly one signature and a checklist item cites exactly one proof, so both hold a pointer. An applicant has many files and no single "the photograph", so `applicants` holds nothing. **Do not derive "the current X" from a `?owner=` filter**: two independent uploads both satisfy "current and non-archived", and the tie would break on row ordering.
- **`document_templates` and `uploaded_files` are the project's only bidirectional app pair.** The file ledger imports that module's selector to resolve a signatory owner; that module calls the ledger's upload service to store a signature. Every other cross-app edge in the project runs one way. Both apps' §2 sections carry the edge, and the invariant keeping the import graph acyclic is written at both import sites.
- **A file cited as checklist evidence, or linked as a signature, can no longer be removed.** Both `evidence_file` and `signature_file` are `PROTECT`, so the database refuses to delete a file either points at. There is no delete endpoint in `uploaded_files` anyway, but these are the only hard constraints of their kind and they will surface if a future cleanup path is ever built.
- **One URL field remains unmigrated; the other was replaced additively.** `clients.logo_url` is still a link to a host this API knows nothing about — `Client` is not one of `uploaded_files`' owner types, so there is nowhere to attach it. `document_templates.signature_image_url` **is** now backed by real file storage: `signatory` became the ledger's sixth owner type on 2026-09-10 and `POST /api/v1/document-templates/signatories/<id>/signature/` uploads an image. The URL field was **kept**, not repointed, so no shipped response shape broke; a new `signature_source` field says which of the two a client should render. That is the pattern to copy for `clients.logo_url` if it is ever done.
- **A signature is fetched, not linked.** There is no public URL for any stored byte in this project — see the deployment guide's standing prohibition on serving `MEDIA_ROOT`. A certificate renders a signature by fetching `/api/v1/files/<id>/download/` with the bearer token and building a blob URL; an `<img src>` pointed at that path returns 401. The route also forbids caching and audits every read, so clients are expected to fetch each signature once per session and hold the object URL.
- **There is no owner rollup.** "Every file related to this applicant" spans the applicant, each of their journeys, each offer on each journey, and each document and snapshot (signatory-owned files belong to no applicant at all) — one request per owner id, merged client-side. There is no `owner_type` filter and no multi-value filter syntax anywhere in that app.
