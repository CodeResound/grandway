...
- **One named document domain is still unbuilt, and all three document modules are incomplete without it.** `uploaded_files` (supporting files, and any generated PDF) has no code and no concept file. A client can now create, edit, archive, restore, **print, review print history, reprint, recover, maintain a signatory library, and retire a template** — but **no file can be attached or stored anywhere**: a document has no attachment field, a snapshot has no file field, and a signature is a link to a host this API knows nothing about. See `document_templates/docs/INTEGRATION.md` §9 and `document_history/docs/INTEGRATION.md` §9.
