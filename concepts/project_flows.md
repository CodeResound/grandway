...
- `document_templates` → `authenticate`, `audit`, and `documents` **at the Python level only** (it imports that app's family enum, slug validator, and key/family rule so the two cannot disagree). **No database relation to any business app**, and nothing consults it: both libraries it publishes — the template catalogue and the signatory list — are advisory.
...
- **A file journey now exists, but it is not yet part of any end-to-end journey.** `uploaded_files`
  shipped on 2026-07-24 (`concepts/uploaded_files_flows.md`) and a passport, transcript, offer
  letter, or document attachment can be uploaded, reviewed, replaced, archived, and downloaded. What
  keeps it out of the journeys above is not capability but **binding**: no other module carries a
  file reference, so attaching a document is a parallel act against `/api/v1/files/` rather than a
  step in a stage. It becomes a real journey step the moment an applicant, an offer, or a document
  gains a first-class file field.
...
- **Files are stored now; nothing points at them.** `uploaded_files` shipped on 2026-07-24 and is the only place in Grandway where bytes exist. Every one of the four gaps recorded here previously is closed on the storage side: a generated PDF, a document attachment, an offer letter, and an applicant photograph all have a home. **What is still missing is the reverse reference** — no module returns a file id, so every files panel is a separate call the screen joins itself. Two URL fields (`clients.logo_url`, `document_templates.signature_image_url`) were deliberately not migrated, and neither `Client` nor `Signatory` is an accepted owner type, so a logo and a signature image are still links to hosts the backend knows nothing about.
- **The document stack's Admin-only rule now reaches into the file ledger, and this is worth knowing before designing navigation.** A file owned by a `document` or a print snapshot is Admin-only, inheriting that module's rule — for a Lead Manager those files are invisible in lists and 404 on every route. Without it, the file ledger would have been a side door around the access decision recorded above. If Lead Managers are ever given document access, that rule must be revisited in the same session.
- **`document_templates` shipped much smaller than its concept file describes, and deliberately.** The frontend owns the templates as code — its 53 slugs are a hardcoded union and its content shapes are compiled-in types — and makes exactly one call into the domain, for the signatory picker. So the version chain, section metadata, field hints, and preview were not built; the signatory library and a thin slug catalogue were. `concepts/document_templates.txt` carries correction notes naming the three screens that therefore have no backing endpoints.
