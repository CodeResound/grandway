...
- **One document-adjacent domain remains unbuilt: `uploaded_files`.** It has no concept file. The consequence is now sharper than it was, because everything around it works: a document can be prepared, printed, reprinted, and recovered, and a certificate can name a real signatory — but **no file can be stored anywhere in this system.** A generated PDF has nowhere to live, a document carries no attachment, and a signature image is a link to a host the backend knows nothing about.
