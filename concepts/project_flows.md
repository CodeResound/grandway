...
- **Two document-adjacent domains remain unbuilt.** `document_templates` (template definitions and signatory records) and `uploaded_files` (supporting files). Neither has a concept file. Until they exist, a certificate cannot resolve its signatory and a document cannot carry an attachment — and note the sharper consequence now that printing works: **a generated PDF has nowhere to live.** A print snapshot records what was issued, but the file itself cannot be stored or referenced anywhere in the system.
