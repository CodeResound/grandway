...
- `document_history` → `documents` (every snapshot and print event needs one, and a recovery **writes** back through that app's update service), `authenticate`, `audit`. **Not** `applicants`: a snapshot reaches the person only through its document. The arrow is one-way — `documents` needs nothing from it.
- `document_templates` → `authenticate`, `audit`, `documents` **at the Python level only** (it imports that app's family enum, slug validator, and key/family rule so the two cannot disagree), and **`uploaded_files` at the database level** — `Signatory.signature_file` is its one relation to a business app, and the file ledger points back at it as an owner type, making the pair the project's only two-way edge. Both libraries it publishes — the template catalogue and the signatory list — remain advisory: nothing validates what a document names.

- `notifications` → `authenticate` (the recipient — its only database relation to anything), `audit`, and **read-only reaches into** `checklists`, `offers`, `applicants`, `uploaded_files`, and `applicant_journeys`. Every one of those five is referenced by app name and a bare id string rather than a foreign key, so an alert can outlive, or precede, the module it points at. **Nothing depends on `notifications` in return** — which is the property that keeps a failure to alert from ever becoming a failure to save the thing being alerted about.
...
- **A notification journey now exists, but it is a parallel track rather than a stage of any journey above.** `notifications` shipped on 2026-07-24 (`concepts/notifications_flows.md`): passport expiry, checklist deadlines, offer response deadlines, uncollected documents, assignment, file rejection, and journey lifecycle all raise real alerts, and the passport `expiry_date` stored in anticipation is now read by a nightly sweep. What keeps it out of the journeys above is its shape — **it is downstream of every stage and part of none.** An alert is raised *because* a journey reached a stage or a deadline passed, and acting on it means leaving this module for the one that owns the work. It also cannot become a journey step in the ordinary way: **the feed is scoped to the calling user**, so "the applicant's alerts" is not a thing any screen can show — only "my alerts about this applicant".
- **No checklist journey.** `checklists` is a named domain in `project_overview.txt` and is not built.
- **A file journey now exists, and one module has bound to it.** `uploaded_files` shipped on
  2026-07-24 (`concepts/uploaded_files_flows.md`) and a passport, transcript, offer letter, or
  document attachment can be uploaded, reviewed, replaced, archived, and downloaded. What kept it
  out of the journeys above was never capability but **binding**: without a file reference on the
  owning record, attaching a document is a parallel act against `/api/v1/files/` rather than a step
  in a stage.
  **Two modules have now bound to it** — `checklists` cites an `evidence_file`, and as of
  2026-09-10 `document_templates` names a signatory's `signature_file` and uploads it through its
  own endpoint. The applicant photograph, the offer letter, and the document attachment still have
  not, so those panels remain parallel acts. The pattern to copy is in
  `concepts/document_templates_flows.md`.
- **`documents` and `document_history` together now cover prepare-and-issue, but still no end-to-end journey passes through them — because of access, not capability.** The workspace is complete for creating, editing, archiving, and restoring (`concepts/documents_flows.md`), and printing, print history, reprinting, and recovery all exist (`concepts/document_history_flows.md`). One of the two blockers recorded here on 2026-07-24 is therefore closed. The remaining one is **access**: both modules are Admin-only, reads included, so the Lead Manager who runs the applicant's file cannot open either. Until that is revisited, "prepare and issue an applicant's documents" is an Admin-only side-branch rather than a stage of the main journey, which is why it is not written as one above.
- **Files are stored, and two modules point at them.** `uploaded_files` shipped on 2026-07-24 and is the only place in Grandway where bytes exist: a generated PDF, a document attachment, an offer letter, and an applicant photograph all have a home. **The reverse reference now exists in two places** — `checklists.evidence_file` and `document_templates.signature_file` — and is still missing everywhere else, so most files panels remain a separate call the screen joins itself.
  **Of the two unmigrated URL fields, one was replaced.** `document_templates.signature_image_url` is now backed by real storage: `Signatory` became the ledger's sixth owner type on 2026-09-10, and the URL field was **kept rather than repointed**, so nothing shipped broke — a `signature_source` field says which of the two is in force. That is the template for the rest. `clients.logo_url` is unchanged: `Client` is still not an owner type, so a partner logo is still a link to a host the backend knows nothing about.
  **What earns a reverse pointer** is that *which* file is the answer to a question — one signature per signatory, one proof per checklist item. An applicant has many files and no single "the photograph", which is why that one is still open.
- **The document stack's Admin-only rule now reaches into the file ledger, and this is worth knowing before designing navigation.** A file owned by a `document`, a print snapshot, or a **signatory** is Admin-only, inheriting that module's rule — for a Lead Manager those files are invisible in lists and 404 on every route. Without it, the file ledger would have been a side door around the access decision recorded above. If Lead Managers are ever given document access, that rule must be revisited in the same session — though the `signatory` entry should be re-argued separately rather than swept along: it is restricted because a print-ready signature image is a forgery tool, not because it contains applicant data, and that argument survives widening document access.
- **`document_templates` shipped much smaller than its concept file describes, and deliberately.** The frontend owns the templates as code — its 53 slugs are a hardcoded union and its content shapes are compiled-in types — and makes two calls into the domain: the signatory picker, and — since 2026-09-10 — the signature upload and render path. So the version chain, section metadata, field hints, and preview were not built; the signatory library and a thin slug catalogue were. `concepts/document_templates.txt` carries correction notes naming the three screens that therefore have no backing endpoints.
- **Two cross-module references are unvalidated, and both compound.** A document's `template_key` names a row in the template catalogue and its `content.instructorId` names a signatory — and `documents` checks neither, because it stores `content` as an opaque body and does not consult the catalogue. **The pickers are the only guard.** Whether either should be enforced is an open decision recorded in `concepts/document_templates_flows.md`.
...
- **An offer letter has a home, but the offer does not know about it.** Since 2026-07-24 the PDF can be stored — `POST /api/v1/files/` with `offer=<id>`, `category=offer_letter` — but an offer payload carries no file reference, so the Offer Detail supporting-files section is a second call keyed on the offer id. Giving `offers` a first-class letter reference is the change that would fold this into the journey proper.
...
- **Deadline and expiry alerting exists now, and one thing about it is worth stating plainly: it does not change any status.** Since 2026-07-24 a nightly sweep raises alerts for a lapsed offer deadline, an overdue checklist item, and an expiring passport, so none of them depends on somebody looking at a list. But `notifications` owns no business state: an offer whose deadline passed is still `issued`, not `expired` — the alert tells a person to act, and only a person's decision moves the offer. Anyone reading a status field should not expect the sweep to have touched it.
