...
- **Development now follows the main/dev pull-request cycle** (PATCH) — permanent `main` (releases) and `dev` (integration); typed task branches `feature|fix|refactor|chore/<name>_<YYYYMMDD_HHMM>` that PR into `dev` and are squash-merged; temporary `release/<N.M.P>` branches from `dev` and `hotfix/*` branches from `main` that PR into `main` with a merge commit, after which `main` is merged back into `dev`. `.githooks/pre-push` refuses direct pushes to `main`/`dev` and gates `release/*`/`hotfix/*` behind `scripts/ci.sh`; `scripts/git_audit.sh` reports drift (merge-back pending, version chain, stale branches, protection). `.github/` gained `workflows/staging.yml` (release-candidate bundle on every push to `dev`), `workflows/production.yml` (renamed from `release.yml`; now also asserts the tag is annotated, on `main`, and has a changelog heading), `branch-protection/{main,dev}.json`, `repo-settings.json`, `pull_request_template.md`, and `CODEOWNERS`; `ci.yml` runs on pull requests into `dev` and `main`. The long-lived `release/1.0.x` maintenance branch is retired.
